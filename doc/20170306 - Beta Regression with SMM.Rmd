---
title: "20170306 Beta Regression directly in Stan"
output: html_notebook
---

Does it make any difference if I regress against SMM instead of CPR? Probably not.
...
sample_data[1:1000,] %>% ggpairs(columns=3:13, mapping=aes(alpha=0.05))
```

Let's try centering Incentive. All of our data is post-crisis, so incentive is rarely negative.
```{r}
sample_data %>% ggplot(aes(x=incentive)) + geom_histogram(bins=50) +
  ggtitle(median(sample_data$incentive))
...
```
Nope, incentive is still negative.
